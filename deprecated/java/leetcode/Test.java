package cia.arkrypto.auth.leetcode;

import cia.arkrypto.auth.leetcode.ds.ListNode;
import cia.arkrypto.auth.leetcode.ds.Node;
import cia.arkrypto.auth.leetcode.ds.Position;
...
//            System.out.println(s);
    ...
    // 返回 left 和 right 之间的所有素数
    public static int[] getPrimes(int left, int right) {
        List<Integer> dp = new ArrayList<>();
        dp.add(2); dp.add(3);
        for(int i = 4; i <= right; i++){
            boolean flag = true;
            for(int j: dp){
                if(j > Math.sqrt(i)){
                    break;
    ...
    public static void shiftColor(){
        ...
        List<Integer> initial = new ArrayList<>(), target = new ArrayList<>();
        for(int i = 0; i < n; i++){
        ...
        long count = 0;
        for(int i = 0; i < n; i++){
            if(!Objects.equals(initial.get(i), target.get(i))){
                int k = target.get(i) - initial.get(i);
                if(k < 0){
                    k += 3;
                }
...
//                    System.out.println(i);
    ...
    public static void change(List<Integer> tree, int root){
        if(root >= tree.size()){
            return;
        }
        if(tree.get(root) == 1){
            tree.set(root, 2);
        } else if(tree.get(root) == 2){
            tree.set(root, 3);
        } else if(tree.get(root) == 3){
            tree.set(root, 1);
        }
        change(tree, 2*root+1);
        change(tree, 2*root+2);
    }


    public static void getKPosition(){
        Scanner scanner = new Scanner(System.in);
        ...
        res.sort((a, b)->{
            int near = (Math.abs(a.x-center.x) + Math.abs(a.y-center.y)) - (Math.abs(b.x-center.x) + Math.abs(b.y-center.y));
            if(near != 0){
                return near;
            }
            int X = a.x-b.x;
    ...
    public static int countBalanceNode(Node root){
        return 0;
    }
